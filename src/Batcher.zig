...
pub fn addVertex(count: u32, vertices: [*]PCV, indices: [*]u32, pos: float2) void {
    const size = 32;
    vertices[count * 4] = .{ .position = float4.new(pos.x, pos.y, 0, 1), .tex_coord = float2.new(0, 0), .color = Color.init(1, 1, 1, 1) };
    vertices[count * 4 + 1] = .{ .position = float4.new(pos.x + size, pos.y, 0, 1), .tex_coord = float2.new(1, 0), .color = Color.init(1, 1, 1, 1) };
    vertices[count * 4 + 2] = .{ .position = float4.new(pos.x, pos.y + size, 0, 1), .tex_coord = float2.new(0, 1), .color = Color.init(1, 1, 1, 1) };
    vertices[count * 4 + 3] = .{ .position = float4.new(pos.x + size, pos.y + size, 0, 1), .tex_coord = float2.new(1, 1), .color = Color.init(1, 1, 1, 1) };
